...
#endif
#if CONFIG_EXAMPLE_CONNECT_WIFI
    ESP_LOGI(TAG, "connect example_disconnect wifi");
    example_wifi_shutdown();
    ESP_LOGI(TAG, "connect example_disconnect example_wifi_shutdown");
    ESP_ERROR_CHECK(esp_unregister_shutdown_handler(&example_wifi_shutdown));
    ESP_LOGI(TAG, "connect example_disconnect after esp_unregister_shutdown_handler");
#endif
    return ESP_OK;
}
