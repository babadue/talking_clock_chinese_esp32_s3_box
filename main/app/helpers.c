...
{
    ESP_LOGI(TAG,"-----------get_folder_path--------full_path:%s", path);
    int len = strlen(path);
    int inx = 0;
    // ESP_LOGI(TAG,"-get_folder_path full_path: %s", path);
    char arr[2];
    for (int i=0; i<len; i++)
    {
        arr[0] = path[i];
        arr[1] = '\0';
        // ESP_LOGI(TAG,"-get_folder_path b4 arr: %s", arr);
        if (strcmp(arr,"/") == 0)
        {
            inx = i;
        }
    ...
    path[inx + 1] = '\0';
    // ESP_LOGI(TAG,"-get_folder_path inx: %d", inx);
    // ESP_LOGI(TAG,"-get_folder_path s: %s", path);
    // strcpy(folder_path, full_path);

}

void get_folder_name(char *folders, int inx)
{
    ESP_LOGI(TAG,"-----------get_folder_name----");
